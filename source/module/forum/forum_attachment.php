...
$downloadPrefix = 'eetop.cn'; // add by yangJie
$attachexists = FALSE;
...
//add by yangJie
if ($_G['uid'] && $attach['uid'] != $_G['uid']) {
    $downlog = C::t('forum_download_log')->fetch_by_aid_uid($aid, $_G['uid']);
    if (!$downlog && !$isimage) {
        $filesize = !$attach['remote'] ? filesize($filename) : $attach['filesize'];
        require_once libfile('function/eetop');
        $attachCredit = getCreditByAttachSize($filesize);
        $extCredits = [
        ...
        updatemembercount($attach['uid'], $extCredits, true, 'IAC', $aid, '', 'download file inc', "extCredits2:$attachCredit,extCredits5:$attachCredit");
        $insertData = [
            'aid' => $aid,
            'uid' => $_G['uid'],
            'dateline' => time(),
        ];
        C::t('forum_download_log')->insert($insertData);
    }
}
