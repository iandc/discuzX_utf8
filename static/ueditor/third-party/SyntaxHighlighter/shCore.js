...
;(function()
{
    ...
    function Brush()
    {
        ...
        var keywords =    'abs access add after alias all and architecture ' +
            ...
            'case component configuration constant downto port procedure process pure ' +
            'downto else elsif end entity error exit file ' +
            'for function generate generic group if ' +
            'impure in inertial inout is ' +
            'label library loop map ' +
            'mod nand next nor not null of on ' +
            'or others out package ' +
            'range record register reject rem report ' +
            'return rol ror select severity signal ' +
            'sla sll sra srl subtype then ' +
            'to transport type unaffected until use ' +
...
;(function()
{
    ...
    function Brush()
    {
        ...
        var keywords =    'after append array ' +
            'auto_execok auto_load auto_mkindex auto_reset ' +
            'bgerror break join method open env ' +
            'case catch cd class clock close common concat constructor continue ' +
            'destructor history if incr info inherit interp ' +
            'eof error eval exec exit expr else elseif expect ' +
            'fblocked fconfigure file fileevent flush for foreach format ' +
            'gets glob global variable vwait image ' +
            'lappend lindex linsert list ength load lrange Ireplace Isearch Isort ' +
            'package parray pid private proc protected public puts pwd ' +
            'read regexp regsub rename return option ' +
            'scan seek set socket source split string subst switch spawn ' +
            'unknown unset update uplevel upvar grab grid ' +
            'auto_ execs auto_ index auto_ noexec auto_ noload auto_ path ' +
            'ErrorCode Errorlnfo unknown_ active entry event ' +
            'bell bind bindtags bitmap button focus frame ' +
            'checkbutton clipboard label listbox lower ' +
            'menu menubutton message radiobutton raise ' +
            'pack photo place pack photo place scale scrollbar selection send' +
            'tk_ setPalette tkerror tkvars tkwait toplevel ' +
            'tcl_ library tcl_ patchLevel tcl_ _pkgPath tcl_ _platform tcl_ precision tcl _rcFileName tcl_ rcRsrcName ' +
            'tell time trace tcl_ endOfWord tcl startOfNextWord tcl_ startOfPreviousWord tcl_ wordBreakAfter ';
